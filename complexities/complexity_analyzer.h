...
void ComplexityAnalyzer<Structure>::runReplications(
  ...
  for (size_t i = 0; i < getStepCount(); ++i) {
    sizes.push_back(getStepSize() * (i + 1));
  }
  using replication_vector = std::vector<duration_t>;
  using results_matrix = std::vector<replication_vector>;
  results_matrix results;

  // napln
  for (size_t i = 0; i < getReplicationCount(); ++i) {
    Structure testStructure(structurePrototype);
    replication_vector durations;
    for (size_t i = 0; i < getStepCount(); ++i) {
      growToSize(testStructure, sizes[i]);
