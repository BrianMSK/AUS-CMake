...
template <typename BlockType> class Hierarchy : virtual public AMT {
public:
  ...
  virtual BlockType *accessSon(const BlockType &node,
                               size_t sonOrder) const = 0;
  ...
  virtual void changeSon(BlockType &parent, size_t sonOrder,
                         BlockType *newSon) = 0;
  virtual void removeSon(BlockType &parent, size_t sonOrder) = 0;

  void processPreOrder(const BlockType *node,
                       std::function<void(const BlockType *)> operation) const;
  void processPostOrder(BlockType *node,
                        std::function<void(BlockType *)> operation) const;
  void processLevelOrder(BlockType *node,
                         std::function<void(BlockType *)> operation) const;
...
protected:
  ...
  protected:
    struct DepthFirstIteratorPosition {
      DepthFirstIteratorPosition(BlockType *currentNode,
                                 DepthFirstIteratorPosition *previousPosition)
          : currentNode_(currentNode), currentSon_(nullptr),
            currentSonOrder_(INVALID_INDEX), visitedSonCount_(0),
            currentNodeProcessed_(false), previousPosition_(previousPosition) {}

      DepthFirstIteratorPosition(const DepthFirstIteratorPosition &other)
          : currentNode_(other.currentNode_), currentSon_(other.currentSon_),
...
public:
  ...
  public:
    PostOrderHierarchyIterator(Hierarchy<BlockType> *hierarchy,
                               BlockType *node);
...
public:
  ...
  void processInOrder(const BlockType *node,
                      std::function<void(const BlockType *)> operation) const;
  ...
  public:
    InOrderHierarchyIterator(BinaryHierarchy<BlockType> *hierarchy,
                             BlockType *node);
...
template <typename BlockType> size_t Hierarchy<BlockType>::nodeCount() const {
  size_t result = 0;
  this->processPreOrder(this->accessRoot(),
                        [&result](const BlockType *b) { result++; });
  return result;
}
...
size_t Hierarchy<BlockType>::nodeCount(const BlockType &node) const {
  size_t result = 0;
  this->processPreOrder(&node, [&result](const BlockType *b) { result++; });
  return result;
}
...
template <typename BlockType>
bool Hierarchy<BlockType>::isNthSon(const BlockType &node,
                                    size_t sonOrder) const {
  BlockType *father = accessParent(node);
  return father != nullptr && accessSon(*father, sonOrder) == &node;
}
...
template <typename BlockType>
bool Hierarchy<BlockType>::hasNthSon(const BlockType &node,
                                     size_t sonOrder) const {
...
void Hierarchy<BlockType>::processPreOrder(
  ...
  if (node != nullptr) {
    operation(node);
    int degreeLoc = degree(*node);
    int orderLoc = 0;
    int numOfProcSons = 0;
    while (numOfProcSons < degreeLoc) {
      BlockType *son = accessSon(*node, orderLoc);
      if (son != nullptr) {
...
void Hierarchy<BlockType>::processPostOrder(
    BlockType *node, std::function<void(BlockType *)> operation) const {
  if (node != nullptr) {
    int degreeLoc = degree(*node);
    int orderLoc = 0;
    int numOfProcSons = 0;
...
Hierarchy<BlockType>::DepthFirstIterator::DepthFirstIterator(
    Hierarchy<BlockType> *hierarchy)
    : hierarchy_(hierarchy), currentPosition_(nullptr) {}
  ...
  for (DepthFirstIteratorPosition *otherPosition = other.currentPosition_;
    ...
    if (currentPosition_ == nullptr) {
      ...
      if (myPosition != nullptr) {
        myPosition->previousPosition_ =
            new DepthFirstIteratorPosition(*otherPosition);
...
bool Hierarchy<BlockType>::DepthFirstIterator::operator==(
    const DepthFirstIterator &other) const {
  ...
  if (myPosition != nullptr && otherPosition != nullptr) {
    if (myPosition->currentNode_ != otherPosition->currentNode_ ||
        myPosition->currentSonOrder_ != otherPosition->currentSonOrder_) {
      return false;
    }
  }
...
template <typename BlockType>
void Hierarchy<BlockType>::DepthFirstIterator::savePosition(
    BlockType *currentNode) {
  currentPosition_ =
      new DepthFirstIteratorPosition(currentNode, currentPosition_);
}
...
template <typename BlockType>
bool Hierarchy<
    BlockType>::DepthFirstIterator::tryFindNextSonInCurrentPosition() {
  ...
  if (currentPosition_->visitedSonCount_ <= currentDegree) {
    do {
      ...
      currentPosition_->currentSon_ = hierarchy_->accessSon(
          *currentPosition_->currentNode_, currentPosition_->currentSonOrder_);
...
Hierarchy<BlockType>::PreOrderHierarchyIterator::PreOrderHierarchyIterator(
    Hierarchy<BlockType> *hierarchy, BlockType *node)
...
template <typename BlockType>
Hierarchy<BlockType>::PreOrderHierarchyIterator::PreOrderHierarchyIterator(
    const PreOrderHierarchyIterator &other)
...
template <typename BlockType>
typename Hierarchy<BlockType>::PreOrderHierarchyIterator
Hierarchy<BlockType>::begin() {
  return PreOrderHierarchyIterator(this, this->accessRoot());
}

template <typename BlockType>
typename Hierarchy<BlockType>::PreOrderHierarchyIterator
Hierarchy<BlockType>::end() {
  return PreOrderHierarchyIterator(this, nullptr);
}

template <typename BlockType>
typename Hierarchy<BlockType>::PreOrderHierarchyIterator
Hierarchy<BlockType>::beginPre() {
  return PreOrderHierarchyIterator(this, this->accessRoot());
}

template <typename BlockType>
typename Hierarchy<BlockType>::PreOrderHierarchyIterator
Hierarchy<BlockType>::endPre() {
  return PreOrderHierarchyIterator(this, nullptr);
}

template <typename BlockType>
typename Hierarchy<BlockType>::PostOrderHierarchyIterator
Hierarchy<BlockType>::beginPost() {
  return PostOrderHierarchyIterator(this, this->accessRoot());
}

template <typename BlockType>
typename Hierarchy<BlockType>::PostOrderHierarchyIterator
Hierarchy<BlockType>::endPost() {
  return PostOrderHierarchyIterator(this, nullptr);
}

template <typename BlockType>
BlockType *
BinaryHierarchy<BlockType>::accessLeftSon(const BlockType &node) const {
  return this->accessSon(node, LEFT_SON_INDEX);
}

template <typename BlockType>
BlockType *
BinaryHierarchy<BlockType>::accessRightSon(const BlockType &node) const {
...
template <typename BlockType>
void BinaryHierarchy<BlockType>::changeLeftSon(BlockType &parent,
                                               BlockType *newSon) {
  this->changeSon(parent, LEFT_SON_INDEX, newSon);
}

template <typename BlockType>
void BinaryHierarchy<BlockType>::changeRightSon(BlockType &parent,
                                                BlockType *newSon) {
...
BinaryHierarchy<BlockType>::InOrderHierarchyIterator::InOrderHierarchyIterator(
    BinaryHierarchy<BlockType> *hierarchy, BlockType *node)
    : Hierarchy<BlockType>::DepthFirstIterator::DepthFirstIterator(hierarchy) {
  if (node != nullptr) {
    this->savePosition(node);
    ++(*this);
...
template <typename BlockType>
BinaryHierarchy<BlockType>::InOrderHierarchyIterator::InOrderHierarchyIterator(
    const InOrderHierarchyIterator &other)
    : Hierarchy<BlockType>::DepthFirstIterator::DepthFirstIterator(other) {}
...
template <typename BlockType>
bool BinaryHierarchy<
    BlockType>::InOrderHierarchyIterator::tryToGoToLeftSonInCurrentPosition() {
  this->currentPosition_->currentSon_ =
      this->hierarchy_->accessSon(*this->currentPosition_->currentNode_,
...
template <typename BlockType>
bool BinaryHierarchy<
    BlockType>::InOrderHierarchyIterator::tryToGoToRightSonInCurrentPosition() {
  this->currentPosition_->currentSon_ =
      this->hierarchy_->accessSon(*this->currentPosition_->currentNode_,
...
template <typename BlockType>
typename BinaryHierarchy<BlockType>::InOrderHierarchyIterator
BinaryHierarchy<BlockType>::begin() {
  return InOrderHierarchyIterator(this, this->accessRoot());
}

template <typename BlockType>
typename BinaryHierarchy<BlockType>::InOrderHierarchyIterator
BinaryHierarchy<BlockType>::end() {
