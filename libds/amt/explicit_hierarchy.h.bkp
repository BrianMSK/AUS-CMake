...
struct ExplicitHierarchyBlock : public MemoryBlock<DataType> {
...
template <typename DataType> using EHBlock = ExplicitHierarchyBlock<DataType>;

template <typename BlockType>
class ExplicitHierarchy : virtual public Hierarchy<BlockType>,
...
template <typename BlockType> using EH = ExplicitHierarchy<BlockType>;
...
struct MultiWayExplicitHierarchyBlock
  ...
  MultiWayExplicitHierarchyBlock()
      : sons_(new IS<MultiWayExplicitHierarchyBlock<DataType> *>()) {}
  ~MultiWayExplicitHierarchyBlock() {
    delete sons_;
    sons_ = nullptr;
  }
...
public:
  ...
  void changeSon(BlockType &parent, size_t sonOrder,
                 BlockType *newSon) override;
...
struct KWayExplicitHierarchyBlock : public ExplicitHierarchyBlock<DataType> {
  KWayExplicitHierarchyBlock()
      : sons_(new IS<KWayExplicitHierarchyBlock<DataType, K> *>(K, true)) {}
  ~KWayExplicitHierarchyBlock() {
    delete sons_;
    sons_ = nullptr;
  }
...
template <typename DataType>
struct BinaryExplicitHierarchyBlock : public ExplicitHierarchyBlock<DataType> {
  BinaryExplicitHierarchyBlock() : left_(nullptr), right_(nullptr) {}
  ~BinaryExplicitHierarchyBlock() {
    left_ = nullptr;
    right_ = nullptr;
  }
...
template <typename DataType> using BinaryEH = BinaryExplicitHierarchy<DataType>;

//----------

template <typename BlockType>
ExplicitHierarchy<BlockType>::ExplicitHierarchy() : root_(nullptr) {}
...
AMT &ExplicitHierarchy<BlockType>::assign(const AMT &other) {
  const ExplicitHierarchy<BlockType> &otherHierarchy =
      dynamic_cast<const ExplicitHierarchy<BlockType> &>(other);
...
template <typename BlockType> void ExplicitHierarchy<BlockType>::clear() {
  this->processPostOrder(this->root_, [&](BlockType *b) {
...
template <typename BlockType>
BlockType *
ExplicitHierarchy<BlockType>::accessParent(const BlockType &node) const {
...
template <typename DataType>
size_t
MultiWayExplicitHierarchy<DataType>::degree(const BlockType &node) const {
  return node.sons_->size();
}

template <typename DataType>
auto MultiWayExplicitHierarchy<DataType>::accessSon(const BlockType &node,
...
template <typename DataType>
auto MultiWayExplicitHierarchy<DataType>::emplaceSon(BlockType &parent,
                                                     size_t sonOrder)
    -> BlockType & {
...
template <typename DataType>
void MultiWayExplicitHierarchy<DataType>::changeSon(BlockType &parent,
                                                    size_t sonOrder,
                                                    BlockType *newSon) {
...
template <typename DataType>
void MultiWayExplicitHierarchy<DataType>::removeSon(BlockType &parent,
                                                    size_t sonOrder) {
  MemoryBlock<BlockType *> *sonBlock = parent.sons_->access(sonOrder);
  BlockType *deletedSon = sonBlock->data_;
  this->processPostOrder(deletedSon, [&](BlockType *b) {
    this->memoryManager_->releaseMemory(b);
  });
...
template <typename DataType, size_t K>
auto KWayExplicitHierarchy<DataType, K>::accessSon(const BlockType &node,
                                                   size_t sonOrder) const
    -> BlockType * {
  MemoryBlock<BlockType *> *sonBlock = node.sons_->access(sonOrder);
  if (sonBlock != nullptr) {
...
template <typename DataType, size_t K>
auto KWayExplicitHierarchy<DataType, K>::emplaceSon(BlockType &parent,
                                                    size_t sonOrder)
    -> BlockType & {
  BlockType *newSon = this->memoryManager_->allocateMemory();
...
template <typename DataType, size_t K>
void KWayExplicitHierarchy<DataType, K>::changeSon(BlockType &parent,
                                                   size_t sonOrder,
                                                   BlockType *newSon) {
  MemoryBlock<BlockType *> *sonBlock = parent.sons_.access(sonOrder);
  BlockType *oldSon = sonBlock->data_;
...
template <typename DataType, size_t K>
void KWayExplicitHierarchy<DataType, K>::removeSon(BlockType &parent,
                                                   size_t sonOrder) {
  // TODO 07
...
template <typename DataType>
auto BinaryExplicitHierarchy<DataType>::accessSon(const BlockType &node,
                                                  size_t sonOrder) const
    -> BlockType * {
  switch (sonOrder) {
...
template <typename DataType>
auto BinaryExplicitHierarchy<DataType>::emplaceSon(BlockType &parent,
                                                   size_t sonOrder)
    -> BlockType & {
...
template <typename DataType>
void BinaryExplicitHierarchy<DataType>::changeSon(BlockType &parent,
                                                  size_t sonOrder,
                                                  BlockType *newSon) {
  if (sonOrder == BinaryHierarchy<BlockType>::LEFT_SON_INDEX) {
...
template <typename DataType>
void BinaryExplicitHierarchy<DataType>::removeSon(BlockType &parent,
                                                  size_t sonOrder) {
  if (sonOrder == BinaryHierarchy<BlockType>::LEFT_SON_INDEX) {
...
auto BinaryExplicitHierarchy<DataType>::accessLeftSon(
    const BlockType &node) const -> BlockType * {
  return node.left_;
}
...
template <typename DataType>
auto BinaryExplicitHierarchy<DataType>::insertLeftSon(BlockType &parent)
    -> BlockType & {
  BlockType *newSon =
      AbstractMemoryStructure<BlockType>::memoryManager_->allocateMemory();
  parent.left_ = newSon;
  newSon->parent_ = &parent;
  return *newSon;
}
...
template <typename DataType>
auto BinaryExplicitHierarchy<DataType>::insertRightSon(BlockType &parent)
    -> BlockType & {
  BlockType *newSon =
      AbstractMemoryStructure<BlockType>::memoryManager_->allocateMemory();
...
template <typename DataType>
void BinaryExplicitHierarchy<DataType>::changeLeftSon(BlockType &parent,
                                                      BlockType *newSon) {
...
template <typename DataType>
void BinaryExplicitHierarchy<DataType>::changeRightSon(BlockType &parent,
                                                       BlockType *newSon) {
