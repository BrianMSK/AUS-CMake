...
public:
  ...
  assign(const CompactMemoryManager<BlockType> &other);
...
BlockType *CompactMemoryManager<BlockType>::allocateMemoryAt(size_t index) {
  if (end_ == limit_) {
    // todo optimize with lambda to use smarter allocation, go from exponential,
    // to linear to constants
    this->changeCapacity(2 * this->getAllocatedBlockCount());
  }

  if (end_ - base_ > static_cast<std::ptrdiff_t>(index)) {
    std::memmove(base_ + index + 1, base_ + index,
...
void CompactMemoryManager<BlockType>::releaseMemoryAt(size_t index) {
  ...
  std::memmove(base_ + index, base_ + index + 1,
...
CompactMemoryManager<BlockType> &CompactMemoryManager<BlockType>::assign(
  ...
  if (this != &other) {
    ...
    this->allocatedBlockCount_ =
        other.MemoryManager<BlockType>::allocatedBlockCount_;
...
template <typename BlockType>
bool CompactMemoryManager<BlockType>::equals(
    const CompactMemoryManager<BlockType> &other) const {
  return this == &other ||
         this->getAllocatedBlockCount() == other.getAllocatedBlockCount() &&
             std::memcmp(base_, other.base_, this->getAllocatedBlocksSize()) ==
                 0;
}
