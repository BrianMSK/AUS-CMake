...
template <typename T> class Stack : virtual public ADT {
public:
...
template <typename T> class ImplicitStack : public Stack<T>, public ADS<T> {
public:
  ImplicitStack();
...
template <typename T> class ExplicitStack : public Stack<T>, public ADS<T> {
public:
  ExplicitStack();
...
template <typename T>
ImplicitStack<T>::ImplicitStack() : ADS<T>(new amt::IS<T>()) {}
...
template <typename T>
ExplicitStack<T>::ExplicitStack() : ADS<T>(new amt::SinglyLS<T>()) {}
...
template <typename T> T &ExplicitStack<T>::peek() {
  ...
  if (this->isEmpty()) {
    throw structure_error("Stack is empty!");
  }
  return block->data_;
}
